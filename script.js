// Each project points at its deployed URL. If you're actively developing one
// locally and want to preview an unreleased change instead, swap `live` for
// `local: "http://localhost:PORT"` temporarily — the modal prefers `live`
// when both are present.
const PROJECTS = [
  {
    name: "Monset Designs",
    tag: "Static HTML / CSS / JS",
    description: "Editorial agency template. Layered hero where a building photo overlaps a giant headline, heavy GSAP/Lenis scroll animation throughout.",
  ...
  },
  {
    ...
    description: "Airbnb-style marketing site for unique stays. Hero, featured stays, guest stories.",
  ...
  },
  {
    ...
    description: "Resort marketing site and booking platform backed by a real Supabase project (rooms, menu, gallery). Live booking writes are intentionally disabled.",
  ...
  },
  {
    ...
    description: "Single-page restaurant site with an interactive 3D coverflow hero carousel and staggered scroll animations.",
...
PROJECTS.forEach((project, i) => {
  ...
  card.innerHTML = `
    ${project.thumb
      ...
      : `<div class="thumb placeholder">${project.name}</div>`
    }
    <div class="body">
      <p class="tag">${project.tag}</p>
      <h3>${project.name}</h3>
      <p>${project.description}</p>
      <p class="cta">View live →</p>
    </div>
  `;
...
const TESTIMONIALS = [
  {
    quote: "They rebuilt our site and bookings in the same month. Leads that used to sit in a shared inbox now land straight in our calendar.",
    author: "Mara Osei",
    role: "Founder, Hunahuna Beach Resort",
  },
  {
    quote: "Search traffic doubled in the first quarter, and it's actually converting, not just visits, real bookings.",
    author: "Daniel Reyes",
    role: "Owner, Restaurant Concept",
  },
  {
    quote: "The dashboard finally gives us one place to see every lead and every job. No more chasing spreadsheets.",
    author: "Priya Nathan",
    role: "Operations Lead, Roost",
  },
];
...
const TOOLS = [
  ...
  { name: "Gemini", color: "#4285F4", icon: '<path d="M11.04 19.32Q12 21.51 12 24q0-2.49.93-4.68.96-2.19 2.58-3.81t3.81-2.55Q21.51 12 24 12q-2.49 0-4.68-.93a12.3 12.3 0 0 1-3.81-2.58 12.3 12.3 0 0 1-2.58-3.81Q12 2.49 12 0q0 2.49-.96 4.68-.93 2.19-2.55 3.81a12.3 12.3 0 0 1-3.81 2.58Q2.49 12 0 12q2.49 0 4.68.96 2.19.93 3.81 2.55t2.55 3.81"/>' },
  { name: "GoHighLevel", icon: "assets/logos/gohighlevel.png" },
  { name: "PayMongo", icon: "assets/logos/paymongo.png" },
  { name: "Supabase", color: "#3ECF8E", icon: '<path d="M11.9 1.036c-.015-.986-1.26-1.41-1.874-.637L.764 12.05C-.33 13.427.65 15.455 2.409 15.455h9.579l.113 7.51c.014.985 1.259 1.408 1.873.636l9.262-11.653c1.093-1.375.113-3.403-1.645-3.403h-9.642z"/>' },
...
const marqueeItemsHtml = TOOLS.map((tool) => {
  ...
  const iconHtml = isImagePath
    ...
    : tool.icon
      ...
      : `<span class="tool-badge-text" aria-hidden="true">${tool.mono}</span>`;
  return `
...
marqueeTrack.innerHTML = marqueeItemsHtml + marqueeItemsHtml;
...
function updateNavScrolled() {
  ...
  nav.classList.toggle("nav--scrolled", past || nav.classList.contains("menu-open"));
}
...
/* ============ Hero ridgeline parallax ============ */
const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

if (!prefersReducedMotion) {
  const ridgeLayers = [
    { el: document.querySelector(".ridge-1"), speed: 0.08 },
    { el: document.querySelector(".ridge-2"), speed: 0.16 },
    { el: document.querySelector(".ridge-3"), speed: 0.26 },
    { el: document.querySelector(".ridge-4"), speed: 0.38 },
  ];
  let ticking = false;

  function applyParallax() {
    const progress = Math.min(window.scrollY / heroEl.offsetHeight, 1.4);
    ridgeLayers.forEach(({ el, speed }) => {
      if (el) el.style.transform = `translateY(${progress * -60 * speed}px)`;
    });
    ticking = false;
  }
...
function openModal(project) {
  const url = project.live || project.local;
  modalTitle.textContent = project.name;
  modalNewTab.href = url;
  modalHint.textContent = project.live
    ? "Live preview. If it doesn't load below, some hosts block embedding, so open it in a new tab instead."
    : `Loading from a local dev server. If this is blank, start it first: ${project.runCmd}`;
  modalFrame.src = url;
  modal.hidden = false;
